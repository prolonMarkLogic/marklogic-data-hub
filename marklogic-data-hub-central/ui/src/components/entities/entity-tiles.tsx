        ...
        >
          <Accordion.Item eventKey={entityModels[entityType].entityTypeId}>
            <Card>
              ...
              <Accordion.Body>
                <Tabs defaultActiveKey={`map-${entityType}`} onSelect={(eventKey) => updateView(index, eventKey, entityType)}>
                  {canReadMapping ? <Tab id={`${entityType}-Map`} data-testid={`${entityType}-Map`} eventKey={`map-${entityType}`} onClick={() => updateView(index, "map", entityType)} title="Map" tabClassName={`curateTab`}/>: null}
                  {props.canReadMatchMerge ? <Tab  id={`${entityType}-Match`} data-testid={`${entityType}-Match`} eventKey="match" key={`match-${entityType}`} onClick={() => updateView(index, "match", entityType)} title="Match" tabClassName={`curateTab`}/>: null}
                  {props.canReadMatchMerge ? <Tab id={`${entityType}-Merge`} data-testid={`${entityType}-Merge`} eventKey="merge" key={`merge-${entityType}`} onClick={() => updateView(index, "merge", entityType)} title="Merge" tabClassName={`curateTab`}/>: null}
                  {props.canReadCustom ? <Tab id={`${entityType}-Custom`} data-testid={`${entityType}-Custom`} eventKey="custom" key={`custom-${entityType}`} onClick={() => updateView(index, "custom", entityType)} title="Custom" tabClassName={`curateTab`}/>: null}
                </Tabs>
