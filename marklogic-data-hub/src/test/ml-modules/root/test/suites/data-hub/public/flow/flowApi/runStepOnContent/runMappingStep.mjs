...
const flowApiSjs = require("/data-hub/public/flow/flow-api.sjs");
...
const sjsResult = flowApiSjs.runFlowStepOnContent(flowName, "1", contentArray, options);
validateResult(sjsResult);

function validateResult(result) {
  const outputContentArray = result.contentArray;

  assertions.push(test.assertEqual(1, outputContentArray.length));
